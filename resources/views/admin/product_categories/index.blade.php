        ...
        <div class="col-sm-12">
            <div class="card">
                ...
                <div class="form-group pt-4 px-3 m-0">
                    <a href="{{route('product-categories.create')}}" class="btn btn-success">新增產品類別</a>
                </div>
                <hr>
                <div class="card-body">
                    <table id="my-table" class="table table-bordered table-striped table-hover">
                        ...
                        <tbody>
                            @foreach ($product_categories as $item)
                                <tr>
                                    <td>{{$item->id}}</td>
                                    <td>{{$item->serviceCategories->name}}</td>
                                    <td>{{$item->name}}</td>
                                    <td>
                                        <a href="{{route('product-categories.edit',['id'=> $item->id])}}" class="btn btn-primary">編輯</a>
                                        <button class="btn btn-danger delete-btn">刪除</button>
                                        <form class="d-none" action="{{route('product-categories.destroy',['id' => $item->id])}}" method="post">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
